...
| `PXE_BRIDGE_ALLOWLIST_SEED`        | With admin | --                    | 32-byte hex secret every allowlist leaf salt derives from. SENSITIVE: anyone holding it can test a candidate address against the on-chain allowlist. Losing it is unrecoverable, since the account stores only a root and the set cannot be read back off chain. Warned about, not refused, when read from the env in production: unlike the signing key it cannot move funds |
| `PXE_BRIDGE_ALLOWLIST_RECIPIENTS`  | With admin | --                    | JSON array of `{address, index}`, e.g. `[{"address":"0x...","index":137}]`. Index is the tree position in `[0, 1024)` and must match what the admin actually built, since position is part of the commitment. Assign positions randomly: filling left to right makes the first touch of a position visibly an addition |

## Architecture

Twelve TypeScript source files plus a Noir contract, no framework -- plain `node:http` server with zod validation:
...
- **`aztec-client.ts`** -- `AztecClient` class implementing `IAztecClient`, wrapping Aztec SDK v5. Creates `EmbeddedWallet`, derives account from secret key (SHA-256 domain-separated salt, reduced into the BN254 field -- a raw digest exceeds the modulus ~81% of the time). When `spendingLimitConfig` is provided, creates a `SpendingLimitAccountContract` via `AccountManager.create()`, registers artifact with PXE, stores in WalletDB, and patches `getAccountFromAddress` for tx routing; otherwise uses standard Schnorr account. Deploys account contract on first connect, caches `TokenContract` instances (capped at 100). Calls `assertAllowlistRootCurrent()` before each `createNote` when spending limits are active, which reads `get_allowlist_root` and refuses to send if the configured tree does not reproduce it; nothing is pushed onto the contract before a send, since the entrypoint derives the declared amount, the recipient and the membership witness from the payload it signs. Account deploys carry `DEPLOY_FEE_HEADROOM` over the worst predicted base fee. Secret key zeroed from memory after connect. Transaction timeout of 120s. Logs the derived account address on connect, because the spending-limit account cannot obtain fee juice for itself and topping it up means naming that address. Exports `deriveAccountKeys()`, which the operator scripts use so they cannot drift from the address the bridge deploys, and refuses `feeJuiceClaim` together with `spendingLimitConfig` (see `FEE_CLAIM_WITH_SPENDING_LIMIT_ERROR`).
...
- **`allowlist-tree.ts`** -- The allowlist, as the bridge holds it. Builds a dense height-10 Merkle tree whose leaves are commitments `h(recipient, salt)`, derives a membership witness per recipient, and exports `rootFromSiblingPath()` as the mirror of the circuit's verification. Salts derive from one secret seed rather than being stored per leaf: they have to be secret, since addresses are enumerable and a publicly derivable salt would let anyone recompute a leaf and test candidates against it. Dense because per-leaf salts leave no canonical empty subtrees to short-circuit, which is what bounds the height at 10 rather than something larger -- building costs `2^height` hashes, 0.33s here against 21s at height 16. Four hash values are pinned identically here and in `main.nr`, because the two implementations never meet at runtime and drift would surface only as every transfer reverting.
- **`spending-limit-account.ts`** -- Custom Aztec account contract TypeScript wrapper. `SpendingLimitAccountContract` implements `AccountContract` with a custom entrypoint that includes declared amount/recipient in the signed hash via `poseidon2HashWithSeparator`. The entrypoint derives the amount, the recipient AND the membership witness from the `transfer_to_private` call in the payload it is signing, so declared == actual holds by construction and there is no per-call state for a concurrent send to overwrite. The tree lives in a holder shared by reference with every entrypoint, since `getAccount()` is called afresh on every wallet resolution and is not memoized, so per-instance state is written to an orphan while a sibling entrypoint signs. Sharing one tree across concurrent sends is safe: the witness is unsigned, the leaf is rebuilt from the signed recipient, and the root is revalidated against live storage at inclusion time. Used with the Noir contract in `contracts/spending_limit_account/`.
- **`contracts/spending_limit_account/`** -- Noir account contract (compiled with `aztec compile`, not `aztec-nargo compile` -- the latter is a symlink to plain `nargo`, ships no transpiler, and emits `transpiled: false`, which `loadContractArtifact` rejects). Extends Schnorr with `check_spending_public`: per-tx cap, 25-bucket sliding daily window, a Merkle recipient allowlist compared as one root, single permitted token, timelocked `propose_limits`/`apply_limits`. The allowlist is one storage slot: membership is proven in private against a root and public only checks that root, so the set never goes on chain. `update_recipient` is the whole allowlist API, taking `(index, old_leaf, new_leaf, sibling_path)`; it verifies the path against the current root before recomputing, which constrains it to one leaf per call and means the caller must already know the tree. It is untimelocked, deliberately: addition, removal and substitution are indistinguishable by construction, so one policy has to cover all three, and the 24h notice bought public legibility that a secret set does not have while costing revocation latency. `pause`/`unpause` remains the immediate emergency stop against a compromised signing key, checked in `check_spending_public` so it bites at inclusion time. Every root change invalidates in-flight transactions, additions included; there is no grace window, because accepting a superseded root would also mean accepting a revoked recipient. Artifact is built into `target/` by the `contract` CI job and gitignored, not committed. Must match Aztec SDK version (currently v5.1.0, set in `Nargo.toml`).
...
- On-chain spending limits via custom Noir account contract (per-tx cap, daily volume, Merkle recipient allowlist, timelocked limits)
- The recipient allowlist is a secret set. Only its root is on chain, membership is proven in private, and the salts that make leaves unguessable derive from an operator-held seed
